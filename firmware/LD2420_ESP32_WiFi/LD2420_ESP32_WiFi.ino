...
void setCORS() {
    if (!httpServer.hasHeader("Origin")) return;
    String origin = httpServer.header("Origin");

    // Parse hostname from Origin (e.g., http://192.168.1.100:8080 -> 192.168.1.100)
    int start = origin.indexOf("://");
    if (start != -1) {
        start += 3;
        int end = origin.indexOf(':', start);
        if (end == -1) end = origin.indexOf('/', start);
        if (end == -1) end = origin.length();
        String hostname = origin.substring(start, end);

        IPAddress ip;
        bool isIP = ip.fromString(hostname);
        if ((isIP && (ip[0] == 10 || (ip[0] == 172 && ip[1] >= 16 && ip[1] <= 31) || (ip[0] == 192 && ip[1] == 168) || ip[0] == 127)) ||
            (!isIP && hostname == "localhost")) {
